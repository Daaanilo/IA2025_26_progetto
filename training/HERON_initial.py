...
def train_heron_initial(episodes=300, batch_size=64, episode_length=1000, threshold_episodes=100):
    ...
    for e in range(episodes):
        ...
        if episode_achievements > best_achievement_count:
            ...
            checkpoint_path = checkpoint_dir / f"heron_initial_best_ep{e}_ach{episode_achievements}"
        ...
        if (e + 1) % 10 == 0:
            checkpoint_dir = Path(__file__).parent / 'heron_initial_output' / 'checkpoints'
            checkpoint_dir.mkdir(parents=True, exist_ok=True)
            checkpoint_path = checkpoint_dir / f"heron_initial_ep{e}"
            agent.save(str(checkpoint_path))
            print(f"[Checkpoint] Periodic checkpoint saved: {checkpoint_path}.*")

        print(f"\n[Episode {e}] Done! (HeRoN Initial)")
    ...
    final_model_path = models_dir / "heron_initial_final"
    agent.save(str(final_model_path))
    print(f"[HeRoN Initial] Final model saved: {final_model_path}.*")
...
def export_achievement_statistics_json_old(evaluation_system, output_file="heron_initial_achievement_statistics.json"):
...
if __name__ == "__main__":
    ...
    print("Configuration: LLM active in first 100 steps of each episode")
    print("Three-Agent System: DQNAgent + CrafterHelper + InstructorAgent")
    print("="*80)

    (rewards, native_rewards, shaped_bonus, achievements, moves,
     helper_calls, hallucinations, helper_stats, reward_shaper_stats) = train_heron_initial(
    ...
    print("\n" + "="*80)
    print("HeRoN Initial Training Complete!")
    print("="*80)
