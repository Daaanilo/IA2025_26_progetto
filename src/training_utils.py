...
class TrainingLogger:
    ...
    def compute_statistics(self, metric_name: str, window: int = 100) -> Dict[str, float]:
        """
        ...
        """
        values = [m[metric_name] for m in self.metrics[-window:] 
                 if metric_name in m]
        ...
        if self.metrics:
            for metric_name in ['reward', 'length', 'achievements']:
                if any(metric_name in m for m in self.metrics):
                    summary[f'{metric_name}_stats'] = self.compute_statistics(metric_name, len(self.metrics))
...
class CheckpointManager:
    ...
    def save_checkpoint(self, agent, episode: int, metrics: Dict[str, Any],
...
class MetricsTracker:
    ...
    def update(self, reward: float = None, length: int = None, 
              achievements: int = None, loss: float = None, 
    ...
    def is_improving(self, metric: str, window: int = 100, 
