...
class Helper:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # API configuration (graceful fallback to mock mode when key is absent)
        self.api_key = os.getenv("OPENAI_API_KEY")
        self.mock_mode = False
        if not self.api_key:
            print(
                "[Helper] WARNING: OPENAI_API_KEY not found in environment variables. Running in mock mode."
            )
            self.mock_mode = True
        else:
        ...
        # Prompts (format safely)
        tmpl = helper_config.get("system_prompt", "")
        try:
            self.system_prompt = tmpl.format(num_actions=self.num_actions_per_call)
        except Exception:
    ...
    def suggest_actions(
        ...
        # If in mock mode (no API key / offline), return safe fallback actions
        ...
        for attempt in range(self.max_retries):
            try:
                # Try legacy ChatCompletion call (older openai versions)
                if hasattr(openai, "ChatCompletion"):
                    ...
                    # Parse response
                    # support both legacy and new structures defensively
                    try:
                        content = response.choices[0].message.content
                    except Exception:
                        # fallback to string decode
                        content = getattr(response.choices[0], "text", str(response))
                else:
                    # Newer openai versions may require migration; avoid raising repeatedly
                    raise RuntimeError(
                        "OpenAI client library does not expose ChatCompletion API in this version"
                    )
                ...
                msg = str(e)
                print(f"Helper API call failed (attempt {attempt + 1}/{self.max_retries}): {msg}")

                # If the error indicates the openai package requires migration, switch to mock
                if ("ChatCompletion" in msg and "not supported" in msg) or (
                    "does not expose ChatCompletion" in msg
                ):
                    print("[Helper] Detected openai client incompatibility with ChatCompletion. Falling back to mock mode.")
                    self.mock_mode = True
                    self.failed_calls += 1
                    self.total_calls += 1
                    return self._get_fallback_actions(num_actions)
    ...
    def _format_user_prompt(
        self, state_info: Dict[str, Any], recent_actions: List[str], num_actions: int
    ) -> str:
        ...
        prompt = self.user_prompt_template.format(
            ...
            achievements=(
                ", ".join(unlocked_achievements) if unlocked_achievements else "none"
            ),
