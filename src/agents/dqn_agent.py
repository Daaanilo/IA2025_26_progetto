...
class DQNNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Choose kernels/strides adaptively when observations are small to avoid
        # kernel > input errors on unusual observation shapes.
        c, h, w = observation_shape
        min_dim = min(h, w) if len(observation_shape) >= 3 else min(h, w)

        if min_dim < 8:
            # Small observations: use conservative small kernels and stride=1
            self.conv1 = nn.Conv2d(c, 32, kernel_size=3, stride=1, padding=1)
            self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
            self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        else:
            # Standard Atari-like configuration
...
class DQNAgent:
    ...
    def __init__(
        ...
        self.policy_net = DQNNetwork(observation_shape, num_actions, network_config).to(
            self.device
        )
        self.target_net = DQNNetwork(observation_shape, num_actions, network_config).to(
            self.device
        )
    ...
    def update(self) -> Optional[float]:
        ...
        if (
            ...
            or self.total_steps < self.learning_starts
        ):
            return None
    ...
    # Compatibility helper methods expected by trainer scripts
    def can_train(self) -> bool:
        """
        Return whether the agent is ready to perform a training step.
        Used by external trainers to gate training.
        """
        return (len(self.replay_buffer) >= self.batch_size) and (
            self.total_steps >= self.learning_starts
        )

    def train_step(self) -> Optional[float]:
        """
        Wrapper used by trainer code to run a single training update.
        This will advance step counters (epsilon decay) and perform the optimizer step.
        Returns the loss value if a training update occurred, otherwise None.
        """
        # Advance counters / decay epsilon
        self.step()
    ...
    def update_target_network(self) -> None:
        """
        Explicit method to update/copy policy weights to the target network.
        Some trainer scripts call this at episode end.
        """
        self.target_net.load_state_dict(self.policy_net.state_dict())
