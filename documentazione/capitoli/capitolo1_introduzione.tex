...
\begin{itemize}
    \item \textbf{Adattamento dell'architettura HeRoN}: L'architettura HeRoN è stata estesa dall'environment JRPG a turni all'environment Crafter, un survival game open-world in tempo continuo.
    
    \item \textbf{Fine-tuning del Reviewer}: Il componente Reviewer è stato adattato ai nuovi task specifici di Crafter, generando un dataset appropriato e addestrando il modello per fornire feedback efficaci nel contesto del survival game.
    
    \item \textbf{Modifica del Helper}: Il comportamento del componente Helper è stato modificato affinché generi sequenze di azioni coerenti (3-5 azioni) anziché singole decisioni, permettendo una pianificazione più strategica.
    
    \item \textbf{Implementazione dell'NPC}: È stato sviluppato un agente di Reinforcement Learning basato sull'algoritmo Deep Q-Network (DQN) ottimizzato per le 17 azioni disponibili in Crafter e il suo spazio di stati a 43 dimensioni.
    
    \item \textbf{Valutazione delle prestazioni}: Le prestazioni dell'architettura HeRoN completa sono state valutate quantitativamente rispetto a baseline tradizionali, misurando il numero di achievement sbloccati nei 22 obiettivi disponibili in Crafter.
\end{itemize}
