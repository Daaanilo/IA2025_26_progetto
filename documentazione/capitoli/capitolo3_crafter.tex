...
\begin{itemize}
    ...
    \item \textbf{Acqua (Drink)}: Diminuisce col tempo; se a zero, la salute cala
\end{itemize}
...
\paragraph{Inventario (16 dimensioni)} Statistiche vitali e conteggio degli oggetti:
\begin{verbatim}
...
\paragraph{Posizione (2 dimensioni)}
\begin{itemize}
    \item Coordinata X (normalizzata in [0,1])
    \item Coordinata Y (normalizzata in [0,1])
\end{itemize}

\paragraph{Status (3 dimensioni)}
\begin{itemize}
    \item Discount (1.0 = vivo, 0.0 = morto)
    \item Sleeping (1.0 = sta dormendo, 0.0 = sveglio)
    \item Daylight (valore normalizzato: 0.0 = notte, 1.0 = giorno)
\end{itemize}
...
Per facilitare l'apprendimento, viene implementato un sistema di reward shaping (classe \texttt{CrafterRewardShaper}) che fornisce segnali più frequenti:

\begin{align}
r_{shaped} &= r_{native} + r_{resources} + r_{health} + r_{tools} + r_{death} \\
r_{resources} &= 0.1 \times \Delta_{\text{risorse}} \quad (\text{wood, stone, coal, iron, diamond, sapling}) \\
r_{health} &= 0.02 \times N_{\text{vitals>5}} \quad (\text{health, food, drink} > 5) \\
r_{tools} &= 0.3 \quad (\text{se nuovo tool craftato, max 0.3 per step}) \\
r_{death} &= -1.0 \quad (\text{penalità se health diventa 0})
\end{align}
