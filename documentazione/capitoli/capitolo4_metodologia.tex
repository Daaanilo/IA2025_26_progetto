...
\begin{itemize}
    ...
    \item Temperatura: default LM Studio (non configurata esplicitamente nel codice)
    \item Max tokens: default LM Studio (non configurato esplicitamente nel codice)
...
\hline
\end{tabular}
\caption{Parametri di fine-tuning supervisionato del Reviewer (Fase 1)}
\end{table}

\textbf{Nota}: Questi parametri si riferiscono alla fase di fine-tuning supervisionato. La successiva fase di fine-tuning con PPO utilizza parametri differenti (vedi Sezione ``Estensione: Fine-tuning del Reviewer tramite RL'').
...
\begin{itemize}
    \item Ogni configurazione testata per 300 episodi
