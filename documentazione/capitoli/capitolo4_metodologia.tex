...
\begin{enumerate}
    \item \textbf{Esecuzione episodi}: 150 episodi di gioco con Helper zero-shot (circa 15.000 esempi di training)
    \item \textbf{Registrazione}: Per ogni chiamata Helper, vengono salvati (come in JSONL):
    \begin{itemize}
        \item Stato dell'environment
        \item Sequenza di azioni suggerite 
        \item Achievement sbloccati 
        \item Istruzioni e feedback 
        \item Metadati episodio/step 
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item Flessibilità di re-planning
...
\textbf{Strategia}: Reinforcement Learning puro senza integrazione LLM. Usa sempre DQN con epsilon-greedy, senza consultazione LLM, con reward shaping applicato per facilitare apprendimento.
...
\begin{itemize}
    \item \texttt{ASSISTED\_STEPS = 100}
    \item \texttt{threshold\_episodes = 100}
    \item Reviewer: T5 PPO fine-tuned 
\end{itemize}
...
\textbf{Strategia}: LLM (Helper + Reviewer) con probabilità crescente durante ogni episodio.
...
\hline
Reviewer model & T5 fine-tuned con PPO  \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Helper}: DQN + Helper senza Reviewer
\end{itemize}
...
Weight decay & 0.01 \\
\hline
Helper calls/episodio & ogni 5 step \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Parametri PPO per Fine-Tuning Reviewer (Fase 2)}
\begin{tabular}{|l|l|}
\hline
\rowcolor{gray!20}
\textbf{Parametro} & \textbf{Valore} \\
