...
\begin{itemize}
    \item \textbf{Policy Network}: aggiornata ad ogni step, utilizzata per selezionare le azioni
    \item \textbf{Target Network}: aggiornata periodicamente, utilizzata per calcolare i target Q-values
\end{itemize}
...
\subsection{DQN Baseline (\texttt{DQN\_training.py})}

\textbf{Strategia}: Pure Reinforcement Learning senza integrazione LLM.

\textbf{Logica di decisione}:
\begin{itemize}
    \item Usa sempre DQN con epsilon-greedy
    \item Nessuna consultazione LLM
    \item Reward shaping applicato per facilitare apprendimento
\end{itemize}

\subsection{DQN + Helper (\texttt{dqn\_helper\_training.py})}
...
\textbf{Logica di decisione}:
\begin{verbatim}
if moves < ASSISTED_STEPS and episode < threshold_episodes:
    action = use_helper()  # Genera sequenza LLM
else:
    action = agent.act(state, env)  # DQN autonomo
\end{verbatim}

\subsection{HeRoN Initial (\texttt{HERON\_initial.py})}
...
\textbf{Logica di decisione}: Identica a DQN+Helper, ma con raffinamento Reviewer:
\begin{verbatim}
if moves < ASSISTED_STEPS and episode < threshold_episodes:
    sequence, response = helper.generate_action_sequence(...)
    if use_reviewer:
        feedback = reviewer.generate_suggestion(...)
        # Re-query Helper con feedback Reviewer
        sequence = helper.generate_action_sequence(
            ..., override_prompt=feedback_prompt)
else:
    action = agent.act(state, env)
\end{verbatim}

\subsection{HeRoN Random (\texttt{HERON\_random.py})}
...
\textbf{Logica di decisione}:
\begin{verbatim}
if random() < LLM_PROBABILITY and episode < threshold_episodes:
    # Workflow Helper + Reviewer
else:
    action = agent.act(state, env)
\end{verbatim}

\subsection{HeRoN Final (\texttt{HERON\_final.py})}

\textbf{Strategia}: LLM (Helper + Reviewer) con probabilità crescente durante ogni episodio (threshold decay per-step).

\textbf{Parametri}:
\begin{itemize}
    \item \texttt{KAPPA = 0.01} (decremento threshold per step)
    \item \texttt{threshold\_episodes = 100}
    \item Reviewer: T5 PPO fine-tuned (\texttt{reviewer\_retrained\_ppo})
\end{itemize}
...
if episode < threshold_episodes:
    if random() > threshold:  # Probabilita' LLM = 1 - threshold
        # Workflow Helper + Reviewer
    else:
        action = agent.act(state, env)
    threshold = max(0, threshold - KAPPA)  # Decay per-step
else:
    action = agent.act(state, env)  # DQN autonomo
\end{verbatim}
...
\begin{itemize}
    \item Step 0: threshold = 1.0 $\rightarrow$ 0\% probabilità LLM (DQN esplora)
    \item Step 50: threshold = 0.5 $\rightarrow$ ~50\% probabilità LLM
    \item Step 100+: threshold = 0.0 $\rightarrow$ 100\% probabilità LLM (guidance strategica)
    \item Threshold si resetta a 1.0 all'inizio di ogni nuovo episodio
\end{itemize}
