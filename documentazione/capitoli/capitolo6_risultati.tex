...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Componente} & \textbf{Specifiche} \\ \midrule
CPU & AMD/Intel (specificare) \\
GPU & NVIDIA CUDA-compatible (specificare) \\
RAM & 16 GB \\
OS & Windows 10/11 \\
Python & 3.9 \\
PyTorch & 2.x con CUDA \\
LM Studio & v0.2.x \\
LLM & Llama-3.2-3B-Instruct \\ \bottomrule
\end{tabular}
...
\section{Analisi del Reward Shaping}
...
\section{Efficacia del Reviewer}
...
Helper (prima): [move_right], [do], [move_right], [do], [noop]
Reviewer: "WARNING: Health is critically low! Prioritize 
           eating or sleeping before exploration."
Helper (dopo): [eat_plant], [eat_plant], [sleep], [move_right], [do]
Risultato: Sopravvivenza garantita, +1 achievement (sleep)
\end{verbatim}

\textbf{Caso 2 - Sequenza Tecnologica}:
\begin{verbatim}
State: inventory=[wood:5], achievements=[collect_wood]
Helper (prima): [move_left], [do], [move_right], [do], [noop]
Reviewer: "You have enough wood. Focus on placing table 
           and crafting pickaxe for progression."
Helper (dopo): [place_table], [make_wood_pickaxe], [move_right], 
               [do], [noop]
Risultato: +2 achievement (place_table, make_wood_pickaxe)
\end{verbatim}

\section{Analisi delle Prestazioni Computazionali}
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Operazione} & \textbf{Tempo Medio} & \textbf{Frequenza} \\ \midrule
DQN forward pass & 2 ms & Ogni step \\
DQN training step & 8 ms & Ogni step \\
Helper LLM call & 480 ms & ~40/episodio (early) \\
Reviewer LLM call & 320 ms & ~40/episodio (early) \\
Environment step & 5 ms & Ogni step \\ \midrule
\textbf{Episodio (ep 1-100)} & 35 s & Include LLM \\
\textbf{Episodio (ep 600+)} & 15 s & Solo DQN \\ \bottomrule
\end{tabular}
\caption{Performance computazionali}
\end{table}

\subsection{Training Time Totale}

\begin{itemize}
    \item \textbf{HeRoN Completo}: ~7 ore per 1000 episodi
    \item \textbf{DQN + Helper}: ~6 ore per 1000 episodi
    \item \textbf{DQN Baseline}: ~4 ore per 1000 episodi
\end{itemize}

L'overhead LLM è significativo ma accettabile considerando i miglioramenti in performance.
...
\section{Test di Significatività Statistica}
...
\section{Confronto con Stato dell'Arte}

Confronto con risultati riportati nell'articolo originale Crafter \cite{hafner2021crafter}:
