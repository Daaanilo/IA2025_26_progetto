...
\begin{enumerate}
    \item \textbf{Monitoraggio token}: L'Helper conta il numero di token nella cronologia usando il tokenizer Qwen2.5 (compatibile con Qwen3)
    ...
    \item \textbf{Reset con riassunto episodio}: Invece di scartare tutto il contesto, l'Helper genera un \textbf{riassunto condensato} che include:
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Salute critica} ($health \leq 5$): Se la salute scende sotto soglia critica, la sequenza viene interrotta e l'Helper viene re-interrogato per suggerire azioni di emergenza (mangiare, bere, dormire)
    
    \item \textbf{Salute bassa} ($health < 30\%$): Se la salute è bassa ma non critica, l'Helper viene ri-consultato per bilanciare l'esplorazione con la gestione della sopravvivenza
...
Ad ogni passo, l'Helper aggiorna il suo tracciamento interno:
\begin{verbatim}
