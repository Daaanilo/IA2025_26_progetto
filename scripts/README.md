...
```powershell
python scripts/1_train_npc_baseline.py --config configs/dqn_config.yaml --episodes 1000
```
...
```powershell
python scripts/3_collect_reviewer_data.py --episodes 200 --output data/reviewer_dataset.jsonl --use-baseline-npc models/baseline/npc_baseline_best.pth
```
...
```powershell
python scripts/5_train_heron.py --config configs/heron_config.yaml --episodes 2000 --load_npc models/baseline/npc_baseline_best.pth
```
...
4. Usa `python scripts/0_pipeline_manager.py --status` per diagnostica
