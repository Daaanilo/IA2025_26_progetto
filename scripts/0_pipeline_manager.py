        ...
        self.steps = [
            {
                ...
                'script': '1_train_npc_baseline.py',
            ...
            },
            {
                ...
                'script': '2_optimize_helper_strategy.py',
            ...
            },
            {
                ...
                'script': '3_collect_reviewer_data.py',
                'description': 'Raccoglie dati per fine-tuning del Reviewer',
                'estimated_time': '1-2 ore',
                'outputs': ['data/reviewer_dataset.jsonl'],
                'dependencies': [1, 2]  # Opzionale: può usare baseline NPC
            },
            {
                'id': 4,
                'name': 'Fine-tune Reviewer',
                'script': '4_finetune_reviewer.py',
                'description': 'Addestra Reviewer su dataset raccolto',
                'estimated_time': '2-4 ore',
            ...
            },
            {
                ...
                'script': '6_evaluate_and_compare.py',
    ...
    def print_status(self):
        ...
        for step in self.steps:
            ...
            if not deps_met and step['dependencies']:
                print(f"   ⚠️  Dipendenze non soddisfatte")
        
        print("\n" + "="*80)
        total_steps = len(self.steps)
        completed_count = len(completed)
        progress = completed_count / total_steps * 100
        print(f"Progresso: {completed_count}/{total_steps} steps completati ({progress:.1f}%)")
        print("="*80)
    
    def run_step(self, step_id, extra_args=None):
    ...
    def run_all(self):
        ...
        for step in self.steps:
            ...
            if not success:
                ...
                print(f"  python scripts/0_pipeline_manager.py --step {step['id']}")
                return
...
def parse_args():
    ...
    group.add_argument('--status', action='store_true',
                      help='Mostra stato della pipeline')
    group.add_argument('--step', type=int,
                      help='Esegui uno specifico step (1-6)')
    group.add_argument('--run-all', action='store_true',
                      help='Esegui l\'intera pipeline')
    group.add_argument('--methodology', action='store_true',
                      help='Mostra metodologia completa')
...
def main():
    ...
    if args.methodology:
        ...
        manager.run_step(args.step, extra_args)
    elif args.run_all:
        manager.run_all()
    else:
        ...
        print("\nEsempi:")
        print("  python scripts/0_pipeline_manager.py --status")
        print("  python scripts/0_pipeline_manager.py --step 1")
        print("  python scripts/0_pipeline_manager.py --step 1 --args --episodes 500")
        print("  python scripts/0_pipeline_manager.py --run-all")
