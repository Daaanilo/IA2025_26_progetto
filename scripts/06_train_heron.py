...
Usage (PowerShell):
  ...
  python scripts/06_train_heron.py --config configs/heron_config.yaml --episodes 2000
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--save_dir", type=str, default="models/heron", help="Directory to save models"
    )
    parser.add_argument(
        "--load_npc",
        type=str,
...
class HeRoNTrainer:
    ...
    def __init__(
        ...
        self.reward_shaping_weight = interaction_config.get(
            "reward_shaping_weight", 0.3
        )
    ...
    def _initialize_components(self):
        ...
        self.reviewer = Reviewer(
            self.reviewer_config, device=self.reviewer_config.get("device", "cuda")
        )
    ...
    def get_helper_strategy(
        self, state_info: Dict, recent_actions: List[str]
    ) -> List[str]:
        """
    ...
    def review_strategy(self, state_info: Dict, suggested_actions: List[str]) -> Dict:
        ...
        feedback = self.reviewer.review_actions(
            state_info=state_info, suggested_actions=suggested_actions
        )
        return feedback
        ...
        if step == 0 or (step - self.last_helper_query_step >= self.helper_query_freq):
            ...
            if self.use_reviewer:
                ...
                if rating >= self.acceptance_threshold:
                    self.current_action_plan = review.get(
    ...
    def train_episode(
        self, episode: int, max_steps: int, log_interactions: bool = False
    ) -> Dict:
        """
    ...
    # interactions_log removed: not used, avoid unused-variable warnings
        ...
        while step < max_steps:
            ...
            if self.npc.can_train():
                # train_step may return loss; ignore here
                self.npc.train_step()
...
def main():
    ...
    for episode in range(num_episodes):
        metrics = trainer.train_episode(
            episode=episode, max_steps=max_steps, log_interactions=args.log_interactions
        )
        ...
        print(
            ...
            f"Achievements: {metrics['achievements']}/{NUM_ACHIEVEMENTS} | "
