...
Usage (PowerShell):
  ...
  python scripts/2_optimize_helper_strategy.py --config configs/helper_config.yaml --iterations 100
"""
import os
import sys
import yaml
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Optimize Helper strategy configuration')
    parser.add_argument('--config', type=str, default='configs/helper_config.yaml',
    ...
    parser.add_argument('--test_configs', nargs='+', type=int, default=[3, 5, 7, 10],
                       help='Different action counts to test (e.g., 3 5 7 10)')
    parser.add_argument('--output_dir', type=str, default='data/helper_analysis',
...
def analyze_action_coherence(actions: List[str], env) -> Dict[str, Any]:
    ...
    crafting_actions = ['make_wood_pickaxe', 'make_stone_pickaxe', 'make_iron_pickaxe',
    ...
    metrics['validity_ratio'] = metrics['valid_actions'] / len(actions) if actions else 0
    
    return metrics


def test_helper_configuration(helper: Helper, env, num_actions: int, 
    ...
    for i in range(iterations):
        ...
        if i > 0:
            num_recent = min(i, 10)
            recent_actions = [env.action_names[np.random.randint(0, len(env.action_names))]
...
def main():
    ...
    for num_actions in args.test_configs:
        ...
        if results['validity_ratios']:
            ...
            print(f"  Mean Generation Time: {results['mean_generation_time']:.2f}s ± {results['std_generation_time']:.2f}s")
    ...
    if valid_results:
        ...
        print("\nRecommendations:")
        print(f"1. Set 'num_actions_per_call' to {best_config['num_actions']} in helper_config.yaml")
        print(f"2. For two strategic calls per episode, use {best_config['num_actions']} x 2 = {best_config['num_actions']*2} total actions")
    ...
    # Save results
    output_file = os.path.join(args.output_dir, f'helper_optimization_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
