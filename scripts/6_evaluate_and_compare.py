...
Usage (PowerShell):
  ...
  python scripts/6_evaluate_and_compare.py --heron_model models/heron/heron_best.pth --baseline_model models/baseline/npc_baseline_best.pth --episodes 100
"""
import os
import sys
import yaml
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Evaluate and compare HeRoN performance')
    parser.add_argument('--heron_model', type=str, required=True,
                       help='Path to trained HeRoN model')
    parser.add_argument('--baseline_model', type=str, default=None,
                       help='Path to baseline NPC model (for comparison)')
    parser.add_argument('--config', type=str, default='configs/dqn_config.yaml',
                       help='Path to DQN configuration file')
    parser.add_argument('--heron_config', type=str, default='configs/heron_config.yaml',
    ...
    parser.add_argument('--use_heron_inference', action='store_true',
    ...
    def compute_statistics(self) -> Dict[str, Any]:
        ...
        if self.all_achievements:
            ...
            stats['unique_achievements_unlocked'] = sum(
                1 for ach_stats in achievement_counts.values() 
...
def evaluate_agent(agent: DQNAgent, env, num_episodes: int, 
                   render: bool = False, name: str = "Agent",
                   use_heron: bool = False, helper=None, reviewer=None,
                   heron_config: dict = None) -> EvaluationResults:
    """
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            if use_heron and helper:
                ...
                if episode_length == 0 or (episode_length - last_helper_query_step >= helper_query_freq):
                    state_info = env.get_state_description()
                    recent_actions = [env.action_id_to_name(a) for a in action_history[-10:]]
                    ...
                    if use_reviewer_flag and reviewer:
                        ...
                        if rating >= acceptance_threshold:
                            current_action_plan = review.get('improved_actions', suggested_actions)
                        else:
                            current_action_plan = []
                    else:
...
def print_comparison(heron_stats: Dict, baseline_stats: Dict = None):
    ...
    print(f"  Mean Reward: {heron_stats['mean_reward']:.2f} ± {heron_stats['std_reward']:.2f}")
    print(f"  Reward Range: [{heron_stats['min_reward']:.2f}, {heron_stats['max_reward']:.2f}]")
    print(f"  Mean Episode Length: {heron_stats['mean_length']:.2f} ± {heron_stats['std_length']:.2f}")
    print(f"  Unique Achievements Unlocked: {heron_stats['unique_achievements_unlocked']}/22")
    print(f"  Avg Achievements per Episode: {heron_stats['avg_achievements_per_episode']:.2f}")
    
    # Print baseline results if available
    if baseline_stats:
        print(f"\n{baseline_stats['name']}:")
        print(f"  Episodes: {baseline_stats['num_episodes']}")
        print(f"  Mean Reward: {baseline_stats['mean_reward']:.2f} ± {baseline_stats['std_reward']:.2f}")
        print(f"  Reward Range: [{baseline_stats['min_reward']:.2f}, {baseline_stats['max_reward']:.2f}]")
        print(f"  Mean Episode Length: {baseline_stats['mean_length']:.2f} ± {baseline_stats['std_length']:.2f}")
        print(f"  Unique Achievements Unlocked: {baseline_stats['unique_achievements_unlocked']}/22")
        print(f"  Avg Achievements per Episode: {baseline_stats['avg_achievements_per_episode']:.2f}")
        ...
        reward_improvement = ((heron_stats['mean_reward'] - baseline_stats['mean_reward']) / 
                             abs(baseline_stats['mean_reward']) * 100 if baseline_stats['mean_reward'] != 0 else 0)
        achievement_improvement = heron_stats['unique_achievements_unlocked'] - baseline_stats['unique_achievements_unlocked']
        avg_achievement_improvement = heron_stats['avg_achievements_per_episode'] - baseline_stats['avg_achievements_per_episode']
        
        print(f"  Reward: {reward_improvement:+.2f}%")
        print(f"  Unique Achievements: {achievement_improvement:+d} ({achievement_improvement/22*100:+.1f}%)")
    ...
    sorted_achievements = sorted(achievements.items(), 
                                key=lambda x: x[1]['success_rate'], 
                                reverse=True)
    
    for ach_name, ach_stats in sorted_achievements:
        if ach_stats['total_unlocks'] > 0:
            print(f"  {ach_name:25s}: {ach_stats['success_rate']:5.1f}% "
...
def create_visualizations(heron_stats: Dict, baseline_stats: Dict, 
    ...
    axes[0].bar(names, rewards, yerr=errors, capsize=5, alpha=0.7, color=['#2ecc71', '#e74c3c'])
    axes[0].set_ylabel('Mean Reward')
    axes[0].set_title('Mean Reward Comparison')
    axes[0].grid(axis='y', alpha=0.3)
    
    # Achievement comparison
    heron_achievements = heron_stats['unique_achievements_unlocked']
    baseline_achievements = baseline_stats['unique_achievements_unlocked'] if baseline_stats else 0
    
    axes[1].bar(names, [heron_achievements, baseline_achievements] if baseline_stats else [heron_achievements],
               alpha=0.7, color=['#2ecc71', '#e74c3c'])
    axes[1].set_ylabel('Unique Achievements Unlocked')
    axes[1].set_ylim(0, 22)
    axes[1].set_title('Achievement Unlocking (out of 22)')
    ...
    if baseline_stats:
        ...
        baseline_rates = [baseline_ach[name]['success_rate'] for name in achievement_names]
        
        data = np.array([heron_rates, baseline_rates])
        
        sns.heatmap(data, xticklabels=achievement_names, 
...
def main():
    ...
    if args.use_heron_inference:
        ...
        try:
            ...
            print("Warning: Could not load fine-tuned Reviewer")
    ...
    print(f"  HeRoN Achievements: {heron_stats['unique_achievements_unlocked']}/22")
    
    if baseline_stats:
        reward_diff = heron_stats['mean_reward'] - baseline_stats['mean_reward']
        ach_diff = heron_stats['unique_achievements_unlocked'] - baseline_stats['unique_achievements_unlocked']
        
        print(f"\n  Improvement over Baseline:")
        print(f"    Reward: {reward_diff:+.2f} ({reward_diff/abs(baseline_stats['mean_reward'])*100:+.1f}%)")
